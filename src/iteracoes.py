...
from algoritmo_emparelhamento import gale_shapley_aluno_propoe, gale_shapley_projeto_propoe
...
def executar_10_iteracoes(
    ...
    for i, (lado, ordem) in enumerate(configuracoes):
        ...
        if lado == "aluno":
            ...
            resultado = gale_shapley_aluno_propoe(alunos, projetos)
        else:
            ordem_ids = aplicar_variacao_ordem(list(projetos.keys()), ordem)
            projetos = {pid: projetos[pid] for pid in ordem_ids}
            resultado = gale_shapley_projeto_propoe(alunos, projetos)
